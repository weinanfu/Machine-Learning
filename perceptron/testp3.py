...
def testRbf(data):
    a,b = loadDataSet(data)
    dataArr = a[:1000]
    labelArr = b[:1000]
    dataArr_test = a[900:1000]
    labelArr_test = b[900:1000]
    ...
    errorCount_test,tp,tn,fp,fn= 0,0,0,0,0
    ...
    for i in range(m): #在测试数据上检验错误率
        ...
        prediction.append(sign(predict))
        print(predict)
        if sign(predict) != sign(labelArr_test[i]) and sign(labelArr_test[i]) ==1:
            fn += 1
            errorCount_test += 1
        if sign(predict)  == sign(labelArr_test[i]) and sign(labelArr_test[i]) ==1:
            tp += 1
